...
/**
 * State Observer - Extracts rich game state into a fixed-size array
 * Enhanced to include maze structure, line-of-sight, and multiple critical points
 */
class StateObserver {
    constructor() {
        this.stateSize = 32; // Expanded from 14 to accommodate richer state
    }
    ...
    getState(agent, agentManager) {
        ...
        try {
            ...
            // Local maze structure (8 numbers) - 3x3 grid around agent
            const mazeGrid = this.getMazeGridAroundAgent(agent, agentManager);
            state.push(...mazeGrid); // 8 values for 3x3 grid (excluding center)
            
            // Directional movement capability (4 numbers) - accurate collision detection
            const movementCapability = this.getMovementCapabilities(agent, agentManager);
            state.push(...movementCapability);
            
            // Visible critical points (12 numbers) - up to 3 visible CPs with line-of-sight
            const visibleCPs = this.getVisibleCriticalPoints(agent, agentManager);
            state.push(...visibleCPs); // 4 numbers per CP (x, z, distance, isOwned) * 3 CPs
            
            // Agent performance and global info (6 numbers)
            state.push(agent.getScore() || 0);
            state.push(agentManager.criticalPoints ? agentManager.criticalPoints.length : 0);
            state.push(this.getGlobalClaimedCount(agentManager));
            state.push(this.getAgentDensityNearby(agent, agentManager));
            state.push(agentPos.y || 0); // Agent height (useful for 3D navigation)
            state.push(this.getDistanceToNearestWall(agent, agentManager));
            
            // Ensure we always have exactly the expected number of values
            while (state.length < this.stateSize) {
                state.push(0);
            }
            
            // Trim if we somehow have too many
            if (state.length > this.stateSize) {
                state.splice(this.stateSize);
            }
            
        } catch (error) {
            console.error('Error generating enhanced state:', error);
            // Return safe fallback state
            return new Array(this.stateSize).fill(0);
        }
        
        return state; // Always this.stateSize numbers
    }
